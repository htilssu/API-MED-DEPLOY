...
async def start_diagnois(image: UploadFile = File(...), user_id: Optional[str] = None):
    try:
        ...
        label_scores_percent_test = {label: (score / total_similarity_test) * 100 for label, score in
                                     label_scores_test.items()}
        ...
        if sorted_labels_test[0][0] == "non-infectious diseases":
            raise HTTPException(status_code=400,
                                detail="Không thể chẩn đoán bệnh này vì hiện tại bệnh thuộc vào nhóm bệnh chưa hỗ trợ")
...
async def knowledge_translate(disease_name: str):
    try:
        ...
        return JSONResponse(content={"translated_name": translated_name, "disease_info": search_result},
                            status_code=200)
    except HTTPException as e:
        logger.error(f"Lỗi HTTP: {e.detail}")
        raise e
